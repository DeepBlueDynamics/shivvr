# Training bge-small Inversion Models for shivvr v3
...
Both are T5-base models fine-tuned on (text, embedding) pairs from MSMARCO.
...
- `bge-small-hypothesis/best_model.pt`
- `bge-small-corrector/best_model.pt`

---

## Step 1: Generate BGE-Small Embeddings for MSMARCO
...
def main():
    ...
    output_dir = Path("./embeddings/bge-small-msmarco")
    batch_size = 256
    max_length = 256
    ...
    for i in tqdm(range(0, len(dataset), batch_size)):
        ...
        with torch.no_grad():
            ...
            # Normalize (bge-small uses normalized embeddings)
...
class BGEInversionModel(nn.Module):
    ...
    def forward(
        ...
        # Project embedding: (batch, 384) -> (batch, d_model)
...
class BGECorrectorModel(nn.Module):
    ...
    def forward(
        self,
        target_embedding: torch.Tensor,     # (batch, 384)
        hypothesis_embedding: torch.Tensor,  # (batch, 384)
...
python scripts/export_onnx_full.py \
  ...
  --model_path ./saves/bge-small-hypothesis/best_model.pt \
  --output_dir ./models/hypothesis

python scripts/export_onnx_full.py \
  --model_type corrector \
  --model_path ./saves/bge-small-corrector/best_model.pt \
...
def main():
    ...
    hypothesis_model = BGEInversionModel.from_pretrained("./saves/bge-small-hypothesis/best_model.pt").to(device)
    hypothesis_model.eval()
    
    corrector_model = BGECorrectorModel.from_pretrained("./saves/bge-small-corrector/best_model.pt").to(device)
...
echo "=== Step 1: Generate BGE embeddings for MSMARCO ==="
python scripts/generate_bge_embeddings.py
...
python scripts/generate_hypotheses.py \
    --hypothesis_model_path ./saves/bge-small-hypothesis/best_model.pt
...
```bash
huggingface-cli upload shivvr/bge-small-hypothesis ./models/hypothesis
huggingface-cli upload shivvr/bge-small-corrector ./models/corrector
```
