...
// Vendored from https://github.com/DeepBlueDynamics/lume — minimum closure for
// shivvr's hybrid search needs: Tagger + tokenize + Token + Tag (this file),
// BM25 retrieval (bm25), Aho-Corasick-style regex NFA (regex), and the
// roaring-bitmap/prime-filter helpers BM25 uses (fast_retrieval).
//
// Skipped modules from upstream lume: graph_search, semantic_mesh, spelling,
// inversion, hybrid (HTTP/cache layer), agent, crawl, cli. Pull from upstream
// lume if any of those become useful here.
pub mod bm25;
