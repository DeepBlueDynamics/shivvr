...
def main(args):
    ...
    # Load GTR-T5-base for re-embedding hypotheses
    ...
    for batch_idx, i in enumerate(pbar):
        ...
        with torch.no_grad():
            gtr_outputs = gtr_model(**gtr_inputs)
            hyp_embs = mean_pool(gtr_outputs.last_hidden_state, gtr_inputs["attention_mask"])
